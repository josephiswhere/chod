...
eventController.decreaseSlots = (req, res, next) => {
  // console.log('eventController.decreaseSlots');
...
eventController.increaseSlots = (req, res, next) => {
  // console.log('eventController.increaseSlots');
  const { subID } = req.query;
  const text = `UPDATE events
                SET slots = slots + 1
                FROM subscriptions
                WHERE subscriptions.event_id = events._id
                AND
                subscriptions._id = ($1)`;
  const values = [subID];
  db.query(text, values)
    ...
    .catch((err) => {
      return next({
        log: `Error in increaseSlots middleware: ${err}`,
        message: { err: 'An error occurred' },
      });
    });
};
